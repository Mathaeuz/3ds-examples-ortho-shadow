...
void updateLight()
{
	...
	Mtx_OrthoTilt(&lightProjection, -2, 2, -2, 2, 0.01f, 100.0f, false);
...
int main()
{
	...
	C3D_Tex shadowTex;
	C3D_TexInitShadow(&shadowTex, 128, 128);
	...
	while (aptMainLoop())
	{
		...
		C3D_FrameBegin(C3D_FRAME_SYNCDRAW);
		{
			...
			C3D_FragOpShadow(1.0, 0);
			// PICA200 depth is [-1~0], so we're remapping to [0~1] by doing (Depth*1.0) + 1.0
			C3D_DepthMap(false, 1.0, 1.0);
			...
			// Backface culling
			// Orthographic shadows, and no bias (0.0). *Fine tune bias acording to use case.

			C3D_FragOpMode(GPU_FRAGOPMODE_GL);

			env = C3D_GetTexEnv(0);
			C3D_TexEnvInit(env);
			C3D_TexEnvSrc(env, C3D_RGB, GPU_TEXTURE1, GPU_TEXTURE0, GPU_PRIMARY_COLOR);
			C3D_TexEnvFunc(env, C3D_RGB, GPU_MODULATE);

			C3D_DepthMap(true, -1.0f, 0.0f);
			C3D_CullFace(GPU_CULL_BACK_CCW);
			C3D_TexShadowParams(false, 0.0);
